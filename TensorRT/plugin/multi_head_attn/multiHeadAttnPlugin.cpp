...
QKVPlugin::QKVPlugin(cublasGemmAlgo_t algo_1, cublasGemmAlgo_t algo_2, bool use_h2) : mAlgo_1(algo_1), mAlgo_2(algo_2), use_h2(use_h2) {}
...
size_t
QKVPlugin::getWorkspaceSize(const nvinfer1::PluginTensorDesc *inputs,
                                int32_t nbInputs,
    ...
    return batch * q_len * kv_len * size + batch * q_len * sizeof(float) * (size == 6);
}
...
//    cudaEventRecord(start, stream);
    ...
    if (mFMHA) {
        qkv_flash((const float*) inputs[0], (const float*) inputs[1], (const float*) inputs[2], (float *)outputs[0],
                batch, q_len, kv_len, embed_dim, stream);
    } else {
        qkv((float *)workspace, (const float*) inputs[0], (const float*) inputs[1], (const float*) inputs[2], (float *)outputs[0],
                batch, q_len, kv_len, embed_dim, m_cublas_handle, stream, mAlgo_1, mAlgo_2);
    }
            break;
        case nvinfer1::DataType::kHALF:
            if (mFMHA) {
        if (use_h2) {
                qkv_flash((const __half2*) inputs[0], (const __half2*) inputs[1], (const __half2*) inputs[2], (__half2 *)outputs[0],
                      batch, q_len, kv_len, embed_dim, stream);
            } else {
                qkv_flash((const __half*) inputs[0], (const __half*) inputs[1], (const __half*) inputs[2], (__half *)outputs[0],
                      batch, q_len, kv_len, embed_dim, stream);
            }
    } else {
        if (use_h2) {
                qkv((__half2 *)workspace, (const __half2*) inputs[0], (const __half2*) inputs[1], (const __half2*) inputs[2], (__half2 *)outputs[0],
                    batch, q_len, kv_len, embed_dim, m_cublas_handle, stream, mAlgo_1, mAlgo_2);
            } else {
                qkv((__half *)workspace, (const __half*) inputs[0], (const __half*) inputs[1], (const __half*) inputs[2], (__half *)outputs[0],
                    batch, q_len, kv_len, embed_dim, m_cublas_handle, stream, mAlgo_1, mAlgo_2);
            }
    }
            break;
        case nvinfer1::DataType::kINT8:
            if (mFMHA) {
        qkv_flash_int8((const int8_4*) inputs[0], scale_q, (const int8_4*) inputs[1], scale_k, (const int8_4*) inputs[2], scale_v, (int8_4 *)outputs[0], scale_o,
                      batch, q_len, kv_len, embed_dim, stream);
    } else {
        qkv_int8((int8_4 *)workspace, (const int8_4*) inputs[0], scale_q, (const int8_4*) inputs[1], scale_k, (const int8_4*) inputs[2], scale_v, (int8_4 *)outputs[0], scale_o,
                batch, q_len, kv_len, embed_dim, m_cublas_handle, stream, mAlgo_1, mAlgo_2);
    }
            break;
        default:
...
size_t QKVPlugin::getSerializationSize() const noexcept {
    return serialized_size(mAlgo_1) + serialized_size(mAlgo_2) + serialized_size(mCfgAlgo);
}
...
bool QKVPlugin::supportsFormatCombination(
    ...
    if (pos == 0) {
        return ((inOut[pos].type == nvinfer1::DataType::kFLOAT || inOut[pos].type == nvinfer1::DataType::kHALF || (inOut[pos].type == nvinfer1::DataType::kINT8 && use_int8)) &&
...
void QKVPlugin::attachToContext(
...
void QKVPlugin::configurePlugin(
...
template<class T> void QKVPlugin::chooseAlgo(nvinfer1::DynamicPluginTensorDesc const *in, nvinfer1::DynamicPluginTensorDesc const *out) noexcept {
    ...
    printf("[QKVPlugin::configurePlugin]: *******************Testing Algo 1*******************\n");
    for (cublasGemmAlgo_t algo=CUBLAS_GEMM_DEFAULT; algo <= CUBLAS_GEMM_ALGO23; algo = (cublasGemmAlgo_t) (algo + 1)) {
        cudaEventRecord(start, stream);
        status = cublasGemmStridedBatchedWrap<T>(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N, kv_len, q_len, embed_dim,
                                                     &alpha, B, embed_dim, kv_len * embed_dim,
                                                     A, embed_dim, q_len * embed_dim, &beta, C,
                                                     kv_len, q_len*kv_len, batch, algo);
        cudaEventRecord(end, stream);
        cudaEventSynchronize(start);
        cudaEventSynchronize(end);
        cudaEventElapsedTime(&time, start, end);
    ...
    for (cublasGemmAlgo_t algo=CUBLAS_GEMM_DEFAULT_TENSOR_OP; algo <= CUBLAS_GEMM_ALGO15_TENSOR_OP; algo = (cublasGemmAlgo_t) (algo + 1)) {
        cudaEventRecord(start, stream);
        status = cublasGemmStridedBatchedWrap<T>(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N, kv_len, q_len, embed_dim,
                                                     &alpha, B, embed_dim, kv_len * embed_dim,
                                                     A, embed_dim, q_len * embed_dim, &beta, C,
    ...
    printf("[QKVPlugin::configurePlugin]: *******************Fastest Algo_1 %d, Time: %f*******************\n", mAlgo_1, minTime);
    ...
    minTime = INFINITY;
    printf("[QKVPlugin::configurePlugin]: *******************Testing Algo 2*******************\n");
    for (cublasGemmAlgo_t algo=CUBLAS_GEMM_DEFAULT; algo <= CUBLAS_GEMM_ALGO23; algo = (cublasGemmAlgo_t) (algo + 1)) {
        cudaEventRecord(start, stream);
        status = cublasGemmStridedBatchedWrap<T>(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, embed_dim, q_len, kv_len,
                                                     &alpha, A, embed_dim, kv_len * embed_dim,
                                                     B, kv_len, q_len*kv_len, &beta, C,
                                                     embed_dim, q_len * embed_dim, batch, algo);
        cudaEventRecord(end, stream);
        cudaEventSynchronize(start);
        cudaEventSynchronize(end);
        cudaEventElapsedTime(&time, start, end);
    ...
    printf("[QKVPlugin::configurePlugin]: *******************Fastest Algo_2 %d, Time: %f*******************\n", mAlgo_1, minTime);
    cudaFreeAsync(A, stream);
    cudaFreeAsync(B, stream);
    cudaFreeAsync(C, stream);
    cudaEventDestroy(start);
...
void QKVPlugin::chooseAlgoINT8(nvinfer1::DynamicPluginTensorDesc const *in, nvinfer1::DynamicPluginTensorDesc const *out) noexcept {
    float minTime = INFINITY, time;
    int batch = in[0].desc.dims.d[0];
    int q_len = in[0].desc.dims.d[1];
    int embed_dim = in[0].desc.dims.d[2];
...
IPluginV2DynamicExt *QKVPluginCreator::deserializePlugin(
...
IPluginV2DynamicExt *QKVPluginCreator2::deserializePlugin(
        const char *name, const void *serialData, size_t serialLength) noexcept {
    try {
