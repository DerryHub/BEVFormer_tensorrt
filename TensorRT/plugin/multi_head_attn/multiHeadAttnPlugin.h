...
namespace trt_plugin {
    ...
    public:
        ...
        template<class T> void chooseAlgo(nvinfer1::DynamicPluginTensorDesc const *in, nvinfer1::DynamicPluginTensorDesc const *out) noexcept;
        void chooseAlgoINT8(nvinfer1::DynamicPluginTensorDesc const *in, nvinfer1::DynamicPluginTensorDesc const *out) noexcept;
        void setCfgAlgo(bool cfgAlgo) {
            mCfgAlgo = cfgAlgo;
        }
        void checkFMHA(const int &q_len, const int &k_len, const int &head_dim) {
            if (q_len % 64 == 0 && k_len % 64 == 0 && (head_dim == 32 || head_dim == 64)) {
                mFMHA = true;
            }
        }
