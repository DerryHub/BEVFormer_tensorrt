...
int32_t GridSamplerPlugin::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,
  ...
  const float scale_o = outputDesc[0].scale, scale_i = inputDesc[0].scale, scale_g = inputDesc[1].scale;
  ...
  case DataType::kHALF:
    ...
    break;
      case DataType::kINT8:
          grid_sample_int8((int8_4*)outputs[0], scale_o, (int8_4*)inputs[0], scale_i, (int8_4*)inputs[1], scale_g,
                           &(output_dims.d[0]), &(input_dims.d[0]), &(grid_dims.d[0]),
                           input_dims.nbDims, mMode, mPaddingMode, mAlignCorners, stream);
          break;
  default:
    ...
    if (use_h2 && !m3D) {
      return (inOut[pos].type == nvinfer1::DataType::kFLOAT &&
             ...
             (inOut[pos].type == nvinfer1::DataType::kHALF &&
              ...
              (inOut[pos].type == nvinfer1::DataType::kINT8 && inOut[pos].format == nvinfer1::TensorFormat::kCHW4);
    }
    if (!m3D) {
        return ((inOut[pos].type == nvinfer1::DataType::kFLOAT ||
          inOut[pos].type == nvinfer1::DataType::kHALF) &&
