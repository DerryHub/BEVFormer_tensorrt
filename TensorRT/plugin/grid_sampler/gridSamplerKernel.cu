...
__device__ __forceinline__ int8_4 cubic_interp1d_int8(int8_4 x0, int8_4 x1,
    ...
    int8_4 weight = int8_4(int8_t(coeffs[0] * 127), int8_t(coeffs[1] * 127), int8_t(coeffs[2] * 127), int8_t(coeffs[3] * 127));
...
__global__ void grid_sampler_2d_kernel_int8(
        const int nthreads, const int8_4 *input, const float scale_i, const int8_4 *grid,
        const float scale_g, int8_4 *output, const float scale_o, TensorDesc input_desc, TensorDesc grid_desc,
        TensorDesc output_desc, const GridSamplerInterpolation interpolation_mode,
        const GridSamplerPadding padding_mode, const bool align_corners) {
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        __half2 grid_xy = __floats2half2_rn(grid_xy_q.x * scale_g, grid_xy_q.y * scale_g);
        ...
        if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
            ...
            for (int c = 0; c < C;
                 ++c, inp_ptr_NC += inp_sC, out_ptr_NCHW += out_sC) {
                if (within_bounds_2d(iy_nearest, ix_nearest, inp_H, inp_W)) {
                    const int8_4 & inp = inp_ptr_NC[iy_nearest * inp_sH + ix_nearest * inp_sW];
...
#pragma unroll 4
                ...
                temp = cubic_interp1d_int8(coefficients[0], coefficients[1], coefficients[2],
                                       coefficients[3], __half2float(__high2half(txy)));
...
void grid_sample_int8(int8_4 *output, const float &scale_o, const int8_4 *input, const float &scale_i, const int8_4 *grid, const float &scale_g, int *output_dims,
                      int *input_dims, int *grid_dims, int nb_dims,
                      GridSamplerInterpolation interp, GridSamplerPadding padding,
                      bool align_corners, cudaStream_t stream) {
    TensorDesc input_desc;
    ...
    if (nb_dims == 4) {
        grid_sampler_2d_kernel_int8
        <<<GET_BLOCKS(count), THREADS_PER_BLOCK, 0, stream>>>(
                count, input, scale_i, grid, scale_g, output, scale_o, input_desc, grid_desc, output_desc,
                interp, padding, align_corners);
    } else {
