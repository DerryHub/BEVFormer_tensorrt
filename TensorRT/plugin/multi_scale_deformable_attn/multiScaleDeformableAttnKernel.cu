...
__device__ void ms_deform_attn_im2col_bilinear_int8_h2_(
        const int8_4 *&bottom_data, const int &height, const int &width,
        const int &nheads, const int &channels, const __half2 &w, const __half2 &h, const __half2 &condition, int8_t *output) {
...
__global__ void ms_deformable_im2col_gpu_kernel_int8(
    ...
    const int8_4 *data_attn_weight, float scale_weight,
    const int spatial_size, const int num_heads, const int channels,
    const int num_levels, const int num_query, const int num_point,
...
#pragma unroll
      for (int w_index = 0; w_index < all_points; ++w_index) {
          const float weight = static_cast<float>(data_attn_weight_i8[data_weight_ptr + w_index]) * scale_weight;
          max_weight = max(max_weight, weight);
      }
...
#pragma unroll
        for (int i = 0; i < 4; i++, data_weight_ptr++, data_offset_w_ptr += 2) {
          ...
          const float w_im = loc_w - 0.5f;
            ...
            weights_arr[i] = T2int8(exp(data_attn_weight_i8[data_weight_ptr] * scale_weight - max_weight) * scale_softmax);
        ...
        dp4a((const int32_t *)(values + 4), weights,
             output.y);
        dp4a((const int32_t *)(values + 8), weights,
             output.z);
        dp4a((const int32_t *)(values + 12), weights,
             output.w);
      }
      data_value_ptr += spatial_h * spatial_w * channels * num_heads;
    }
...
#pragma unroll
      for (int w_index = 0; w_index < all_points >> 2; ++w_index) {
          ...
          __half2 weight = __hmul2(__halves2half2(__short2half_rn(weights.x), __short2half_rn(weights.y)), scale_weight_h2);
          max_weight = hmax(max_weight, weight);

          weight = __hmul2(__halves2half2(__short2half_rn(weights.z), __short2half_rn(weights.w)), scale_weight_h2);
          max_weight = hmax(max_weight, weight);
      }
      ...
      for (int point_index = 0; point_index < (num_point >> 2); point_index++) {
        ...
        auto values_i32 = reinterpret_cast<int32_t*>(values);
          ...
          *weights_i84 = data_attn_weight[data_weight_ptr];
          weights_h2[1] = __hmul2(h2exp(__hfma2(
                  __halves2half2(__short2half_rn(weights_i84->z), __short2half_rn(weights_i84->w)),
                  scale_weight_h2, __hneg2(max_weight))), scale_softmax);
          weights_h2[0] = __hmul2(h2exp(__hfma2(
...
#pragma unroll
        for (int i = 0; i < 2; i++) {
            ...
            const int point_index_per_group_1 = ((point_index << 2) + (i << 1) + 1) % points_per_group;
          const __half2 reference_point_xy_0 =
            ...
            const __half2 offset_xy_0 = __hfma2(
                    __halves2half2(
                            __short2half_rn(offsets.x),
                            __short2half_rn(offsets.y)), scale_offset_h2,
                    __float2half2_rn(-0.5f));
            const __half2 offset_xy_1 = __hfma2(
                    __halves2half2(
                            __short2half_rn(offsets.z),
                            __short2half_rn(offsets.w)), scale_offset_h2,
                    __float2half2_rn(-0.5f));

            const __half2 wh_im_0 =
                    __hfma2(reference_point_xy_0,
                            spatial_wh,
                            offset_xy_0);
            const __half2 wh_im_1 =
                    __hfma2(reference_point_xy_1,
                            spatial_wh,
                            offset_xy_1);

            const __half2 condition_0 = __hmul2(__hgt2(wh_im_0, __float2half2_rn(-1.f)), __hlt2(wh_im_0, spatial_wh));
            const __half2 condition_1 = __hmul2(__hgt2(wh_im_1, __float2half2_rn(-1.f)), __hlt2(wh_im_1, spatial_wh));

            const __half2 condition = __halves2half2(condition_0.x * condition_0.y, condition_1.x * condition_1.y);
            ...
            if(condition.y) {
                ms_deform_attn_im2col_bilinear_int8_h2(
                        data_value_ptr, spatial_h, spatial_w, num_heads, channels, wh_im_1, values + 2*i + 1);
            }
  ...
  const int num_kernels = batch_size * num_query * num_heads * channels;
    ms_deformable_im2col_gpu_kernel_int8<scalar_t>
        <<<GET_BLOCKS(num_kernels), THREADS_PER_BLOCK, 0, stream>>>(
            ...
            data_attn_weight, scale_weight, spatial_size, num_heads,
            channels, num_levels, num_query, num_point, points_per_group,
            data_col, scale_out);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
