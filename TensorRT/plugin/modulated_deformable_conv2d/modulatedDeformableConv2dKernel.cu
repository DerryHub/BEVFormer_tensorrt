...
__forceinline__ __device__ void dmcn_im2col_bilinear_int8(const int8_4 *input, const float &scale_i,
                                           const int data_width,
                                           const int height, const int width,
                                           __half2 hw, int8_4 &output) {
    __half2 hw_low = h2floor(hw);
    __half2 hw_high = __hadd2(hw_low, __float2half2_rn(1.f));
    __half2 lhw = __hsub2(hw, hw_low);
    __half2 hhw = __hsub2(__float2half2_rn(1.f), lhw);
...
__global__ void modulated_deformable_im2col_gpu_kernel_int8(
        const int n, const int8_4 *data_im, const float scale_i, const int8_t *data_offset, const float scale_off,
        const int8_t *data_mask, const float scale_mask, const int height, const int width,
        const int kernel_h, const int kernel_w, const int pad_h, const int pad_w,
        const int stride_h, const int stride_w, const int dilation_h,
        const int dilation_w, const int channel_per_deformable_group,
        const int batch_size, const int num_channels, const int deformable_group,
        ...
        for (int i = 0; i < kernel_h; ++i) {
            for (int j = 0; j < kernel_w; ++j) {
                ...
                const __half2 offset_hw = __floats2half2_rn(
                        data_offset_ptr[data_offset_h_ptr] * scale_off, data_offset_ptr[data_offset_w_ptr] * scale_off);
...
__global__ void output_add_bias_kernel_int8(
        const int32_t *int32_iw, float scale_iw, const float* bias, int8_t*output, float scale_o, size_t n, int hw_out) {
    CUDA_1D_KERNEL_LOOP(index, n) {
int bias_index = index / (hw_out);
output[index] = T2int8<float>((int32_iw[index] * scale_iw + bias[bias_index]) / scale_o);
}
}
...
__global__ void output_wo_bias_kernel_int8(
        const int32_t *int32_iw, float scale_iw, int8_t *output, float scale_o, size_t n) {
    CUDA_1D_KERNEL_LOOP(index, n) {
...
void trt_modulated_deformable_im2col_int8(
        const int8_4 *data_im_, const float &scale_i, const int8_t *data_offset_, const float &scale_off,
        const int8_t *data_mask_, const float &scale_mask, const int batch_size, const int channels,
        const int height_im, const int width_im, const int height_col,
        const int width_col, const int kernel_h, const int kenerl_w,
        const int pad_h, const int pad_w, const int stride_h, const int stride_w,
        const int dilation_h, const int dilation_w, const int deformable_group,
        int8_4 *data_col_, cudaStream_t stream) {
    // num_axes should be smaller than block size
    const int channel_per_deformable_group =
    ...
    THREADS_PER_BLOCK, 0, stream>>>(
            num_kernels, data_im_, scale_i, data_offset_, scale_off, data_mask_, scale_mask, height_im, width_im,
            kernel_h, kenerl_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
            dilation_w, channel_per_deformable_group, batch_size, channels,
            deformable_group, height_col, width_col, data_col_);
...
void ModulatedDeformConvForwardCUDAKernel_int8(
        const int8_4 *input, const float &scale_i, const int8_4 *weight, const float &scale_w, const float *bias, const int8_t *offset,
        const float &scale_off, const int8_t *mask, const float &scale_mask, int8_t *output, const float &scale_o, void *workspace, int batch, int channels,
    ...
    auto int32_temp = (int32_t*)(columns + (channels + 3) / 4 * kernel_w * kernel_h * height_out * width_out);
    ...
    for (int b = 0; b < batch; b++) {
        ...
        trt_modulated_deformable_im2col_int8(
                input_start, scale_i, offset_start, scale_off, mask_start, scale_mask, 1, channels, height, width,
                height_out, width_out, kernel_h, kernel_w, pad_h, pad_w, stride_h,
                stride_w, dilation_h, dilation_w, deformable_group, columns, stream);
        ...
        for (int g = 0; g < group; g++) {
            ...
            cublasGemmWrap_int8(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k,
                                   ...
                                   (int8_t *)weight_start, k, &beta, int32_temp,
                                   n);

            if (with_bias) {
                const float *bias_start =  bias + g * m;
                output_add_bias_kernel_int8<<<GET_BLOCKS(mn),
                THREADS_PER_BLOCK, 0, stream>>>(
                        int32_temp, scale_iw, bias_start, out_buffer_start, scale_o, mn, n
                );
            }
            else {
                output_wo_bias_kernel_int8<<<GET_BLOCKS(mn),
                THREADS_PER_BLOCK, 0, stream>>>(int32_temp, scale_iw, out_buffer_start, scale_o, mn);
            }
