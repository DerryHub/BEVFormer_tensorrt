...
__global__ void bev_pool_v2_kernel_h2(int c, int n_intervals,
                                   const __half *__restrict__ depth,
...
__global__ void bev_pool_v2_kernel_int8(int c, int n_intervals,
                                      const int8_t *__restrict__ depth,
                                      const int8_4 *__restrict__ feat,
                                      const int *__restrict__ ranks_depth,
                                      const int *__restrict__ ranks_feat,
                                      const int *__restrict__ ranks_bev,
                                      const int *__restrict__ interval_starts,
                                      const int *__restrict__ interval_lengths,
                                        int8_4* __restrict__ out, const float scale_io) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int index = idx / c;
    int cur_c = idx % c;
    if (index >= n_intervals) return;
...
template <typename T>
void bev_pool_v2(int c, int n_intervals, int num_points, const T* depth, const T* feat, const int* ranks_depth,
                 const int* ranks_feat, const int* ranks_bev, const int* interval_starts, const int* interval_lengths, T* out,
                 cudaStream_t stream) {
    cudaMemset((T *)out, 0, num_points * sizeof(T));
    bev_pool_v2_kernel<<<GET_BLOCKS(n_intervals * c), THREADS_PER_BLOCK, 0, stream>>>(
...
void bev_pool_v2_h2(int c, int n_intervals, int num_points, const __half* depth, const __half2* feat, const int* ranks_depth,
    ...
    bev_pool_v2_kernel_h2<<<GET_BLOCKS(n_intervals * c/2), THREADS_PER_BLOCK, 0, stream>>>(
            c/2, n_intervals, depth, feat, ranks_depth, ranks_feat,
            ranks_bev, interval_starts, interval_lengths, out
    );
    cudaCheckError();
}
...
void bev_pool_v2_int8(int c, int n_intervals, int num_points, const int8_t* depth, const float &scale_d, const int8_4* feat, const float &scale_f, const int* ranks_depth,
    ...
    bev_pool_v2_kernel_int8<<<GET_BLOCKS(n_intervals * c/4), THREADS_PER_BLOCK, 0, stream>>>(
            c/4, n_intervals, depth, feat, ranks_depth, ranks_feat,
            ranks_bev, interval_starts, interval_lengths, out, scale_d * scale_f / scale_o
    );
    cudaCheckError();
}

template void bev_pool_v2(int c, int n_intervals, int num_points, const float* depth, const float* feat, const int* ranks_depth,
                          const int* ranks_feat, const int* ranks_bev, const int* interval_starts, const int* interval_lengths, float* out,
                          cudaStream_t stream);

template void bev_pool_v2(int c, int n_intervals, int num_points, const __half* depth, const __half* feat, const int* ranks_depth,
                          const int* ranks_feat, const int* ranks_bev, const int* interval_starts, const int* interval_lengths, __half* out,
                          cudaStream_t stream);
