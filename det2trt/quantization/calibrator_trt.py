...
def get_calibrator(calibrator):
    assert calibrator in CALIBRATORS, 'calibrator_qdq should be in [minmax, entropy, legacy]'
    ...
    class Calibrator(CALIBRATORS[calibrator]):
        def __init__(self, config, dataloader, length):
            ...
            self.num_batch = min(len(dataloader.dataset) // self.batch_size, self.num_batch)
            self.input_shapes = {}
            ...
            for name in self.names:
                ...
                self.host_device_mem_dic[name] = HostDeviceMem(name, host_mem, device_mem)
            ...
            try:
                ...
                [
                    cuda.memcpy_htod(
                        ...
                        self.host_device_mem_dic[name].host) for name in names
                ]
                self.current_batch += 1
                return [
                    int(self.host_device_mem_dic[name].device) for name in names
                ]
