...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask = None,
        ...
        query = F.linear(query, self.in_proj_weight[:self.embed_dim], self.in_proj_bias[:self.embed_dim])
        key = F.linear(key, self.in_proj_weight[self.embed_dim:self.embed_dim*2], self.in_proj_bias[self.embed_dim:self.embed_dim*2])
        value = F.linear(value, self.in_proj_weight[self.embed_dim*2:], self.in_proj_bias[self.embed_dim*2:])

        query = query.view(-1, batch_size * self.num_heads, self.embed_dim // self.num_heads).permute(1, 0, 2)
        key = key.view(-1, batch_size * self.num_heads, self.embed_dim // self.num_heads).permute(1, 2, 0)
