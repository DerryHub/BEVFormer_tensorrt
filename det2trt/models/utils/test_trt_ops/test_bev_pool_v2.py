...
def get_lidar_coor(sensor2ego, cam2imgs, post_rots, post_trans, bda):
    ...
    d = torch.arange(1.0, 161.0, 1.0, dtype=torch.float).view(-1, 1, 1).expand(-1, H_feat, W_feat)
    x = torch.linspace(0, W_in - 1, W_feat, dtype=torch.float).view(1, 1, W_feat).expand(d.shape[0], H_feat, W_feat)
    y = torch.linspace(0, H_in - 1, H_feat, dtype=torch.float).view(1, H_feat, 1).expand(d.shape[0], H_feat, W_feat)
...
def bev_pool_prepare():
    sensor2keyegos = torch.tensor(
        [[[[8.2076e-01, -3.4143e-04, 5.7128e-01, 1.5239e+00],
           [-5.7127e-01, 3.2195e-03, 8.2075e-01, 4.9463e-01],
           [-2.1195e-03, -9.9999e-01, 2.4474e-03, 1.5093e+00],
           [0.0000e+00, 0.0000e+00, 0.0000e+00, 1.0000e+00]],

          [[6.2286e-03, -5.5937e-03, 9.9996e-01, 1.7707e+00],
           [-9.9998e-01, -8.5861e-04, 6.2239e-03, 1.6166e-02],
           [8.2376e-04, -9.9998e-01, -5.5989e-03, 1.5124e+00],
           [0.0000e+00, 0.0000e+00, 0.0000e+00, 1.0000e+00]],

          [[-8.3232e-01, 4.2987e-05, 5.5429e-01, 1.6938e+00],
           [-5.5422e-01, 1.6328e-02, -8.3221e-01, -4.9285e-01],
           [-9.0865e-03, -9.9987e-01, -1.3567e-02, 1.4986e+00],
           [0.0000e+00, 0.0000e+00, 0.0000e+00, 1.0000e+00]],

          [[9.4656e-01, 8.7274e-03, -3.2241e-01, 1.4300e+00],
           [3.2251e-01, -1.4109e-02, 9.4646e-01, 4.8685e-01],
           [3.7113e-03, -9.9986e-01, -1.6170e-02, 1.5984e+00],
           [0.0000e+00, 0.0000e+00, 0.0000e+00, 1.0000e+00]],

          [[-2.6211e-04, -1.6699e-02, -9.9986e-01, 3.3118e-01],
           [9.9999e-01, -4.0898e-03, -1.9385e-04, 1.7097e-03],
           [-4.0860e-03, -9.9985e-01, 1.6700e-02, 1.5849e+00],
           [0.0000e+00, 0.0000e+00, 0.0000e+00, 1.0000e+00]],

          [[-9.3536e-01, 1.5928e-02, -3.5334e-01, 1.2277e+00],
           [3.5353e-01, 1.1332e-02, -9.3536e-01, -4.8050e-01],
           [-1.0894e-02, -9.9981e-01, -1.6230e-02, 1.5665e+00],
           [0.0000e+00, 0.0000e+00, 0.0000e+00, 1.0000e+00]]]],
        device="cuda"
    )
    ...
    grid_lower_bound = torch.tensor([-100., -100., -5.0000])
    grid_interval = torch.tensor([1., 1., 8.0000])
    grid_size = [200, 200, 1]
    ...
    coor = ((coor - grid_lower_bound.to(coor)) /
...
class BEVPoolV2TestCase(BaseTestCase, unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ranks_bev, ranks_depth, ranks_feat, interval_starts, interval_lengths = bev_pool_prepare()
        cls.input_data = dict(
    ...
    def createInputs(self):
        if self.fp16:
            ...
            self.inputs_pth_fp16 = {key: val.half() if key in ['depth', 'feat'] else val for key, val in inputs_pth.items()}
            self.inputs_np_fp16 = {
